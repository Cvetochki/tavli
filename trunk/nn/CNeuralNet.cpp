...
CNeuralNet::CNeuralNet(int numOfInputs, int numOfHiddenNeurons, int numOfOutputs)
 ...
 m_gameCounter(0)
{
	...
	if (m_hiddenNeurons) {
		...
		std::cout << "Uh oh, no memory for " << numOfHiddenNeurons << " neurons!"<<std::endl;
		return;
	}
	...
	if (m_outNeurons) {
		...
		std::cout << "Uh oh, no memory for " << numOfOutputs << " neurons!"<<std::endl;
		return;
	}
...
CNeuralNet::CNeuralNet(char *filename)
{
	...
	if (m_hiddenNeurons) {
		...
		std::cout << "Uh oh, no memory for " << m_numOfHiddenNeurons << " neurons!"<<std::endl;
		return;
	}
	...
	if (m_outNeurons) {
		...
		std::cout << "Uh oh, no memory for " << m_numOfOutputs << " neurons!"<<std::endl;
		return;
	}
...
int CNeuralNet::loadNet(char *filename)
{
    ...
    if (numOfInputs!=m_numOfInputs) {
    	std::cerr << "Problem in number of inputs when loading weights" << std::endl;
    	return 0;
    }
    if (numOfHiddenNeurons!=m_numOfHiddenNeurons) {
    	std::cerr << "Problem in number of hidden neurons when loading weights" << std::endl;
    	return 0;
    }
    if (numOfOutputs!=m_numOfOutputs) {
    	std::cerr << "Problem in number of outputs when loading weights" << std::endl;
    	return 0;
    }
...
//#define DEBUGTRAIN
	...
	for (int i = 0; i < NUM_OUTPUTS; ++i)
    {
		...
		for (j = 0; j < NUM_NEURONS; ++j)
        {
			...
			m_outNeurons[i].m_w[j] += learningRate * delta_outputs[i] * m_hiddenNeurons[j].m_activation;
        }
	...
	for (int i = 0; i < NUM_NEURONS; i++)
    {
		...
		for (j = 0; j < NUM_INPUTS; j++)
        {
            //use momentum (delta values from last pass),
            //to ensure moved in correct direction
			m_hiddenNeurons[i].m_w[j] += learningRate * delta_hidden[i] * inp[j];
        }
...
//define DEBUGTRAIN
	...
	for(i=0; i<NUM_OUTPUTS; ++i)
		m_outNeurons[i].m_error=(target[i]-out[i])*out[i] * (1 - out[i]) /* * BETA_OUTPUT */;
	...
	for (i=0; i<NUM_OUTPUTS; ++i)
		for(j=0; j<NUM_NEURONS; ++j)
			m_hiddenNeurons[j].m_error+= m_outNeurons[i].m_error * m_outNeurons[i].m_w[j];
	for(i=0; i<NUM_NEURONS; ++i)
		m_hiddenNeurons[i].m_error *= /*BETA_HIDDEN **/ m_hiddenNeurons[i].m_activation * (1 - m_hiddenNeurons[i].m_activation);
		//m_hiddenNeurons[i].m_error *= BETA_HIDDEN * ar[i] * (1 - ar[i]);
	
	/* Adjust weights at output nodes */
	for(i=0; i<NUM_OUTPUTS; ++i) {
		for(j=0; j<NUM_NEURONS; ++j)
			m_outNeurons[i].m_w[j] += learningRate * m_outNeurons[i].m_error * m_hiddenNeurons[j].m_activation;
	...
	for(i=0; i<NUM_INPUTS; ++i)
		if (inp[i]==1.0)
			...
			for(j=0; j<NUM_NEURONS; ++j)
				m_hiddenNeurons[j].m_w[i] +=  learningRate * m_hiddenNeurons[j].m_error * inp[i];
	for(j=0; j<NUM_NEURONS; ++j)
		m_hiddenNeurons[j].m_w[i] += learningRate * m_hiddenNeurons[j].m_error * BIAS;
	++m_trainingCounter;
#ifdef DEBUGTRAIN
	nnFeed(inp);
